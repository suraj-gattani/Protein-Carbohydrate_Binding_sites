...
def main():
	...
	features_file = pd.read_csv('feature_file_test_ws5.csv', header=None)
	...
	etc_prob = pd.read_csv('LOG_test.txt', header=None)
	etc_matrix = etc_prob.as_matrix()
			
	rdf_prob = pd.read_csv('SVM_test.txt', header=None)
	rdf_matrix = rdf_prob.as_matrix()
	print(rdf_prob.shape)
	
	knn_prob = pd.read_csv('ETC_test.txt', header=None)
	knn_matrix = knn_prob.as_matrix()
	print(knn_prob.shape)
			
	gbc_prob = pd.read_csv('XGB_test.txt', header=None)
	...
	np.savetxt('Model_XGB_3_stacking_test_SVM_meta.csv', ekr, delimiter=',', fmt='%s')
